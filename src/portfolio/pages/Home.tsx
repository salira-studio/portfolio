...
const upcoming = [
  {
    icon: HeartPulse,
    name: 'Healthcare',
    blurb: 'Patient and clinic experiences shaped around real care workflows.',
    tag: 'Next build',
  },
  {
    icon: ShoppingBag,
    name: 'Retail',
    blurb: 'Storefronts, inventory and staff tools built as one connected system.',
    tag: 'Research',
  },
  {
    icon: Truck,
    name: 'Logistics',
    blurb: 'Tracking and operations software that mirrors how goods actually move.',
    tag: 'Concept',
  },
]
        ...
        <div className="mx-auto max-w-6xl px-5 py-18 sm:px-8 sm:py-28">
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
            ...
            <p className="max-w-md text-sm text-[var(--sl-ink-soft)]">
              Each entry ships as a working, production-grade system demonstration — never a mock video or static design file.
            </p>
          </div>

          <div className="mt-12 space-y-6">
            {/* Featured Active Card: Restaurants (Shipping now) */}
            <FadeUp delay={0.1}>
              <Link
                to="/work/restaurants"
                data-cursor="view"
              ...
              >
                <div className="grid md:grid-cols-12 items-stretch">
                  <div className="md:col-span-7 flex flex-col p-7 sm:p-10 z-10">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="inline-flex items-center gap-1 rounded-full bg-[rgba(46,111,94,0.1)] px-2.5 py-0.5 text-[10px] font-semibold text-[var(--sl-teal-deep)] ring-1 ring-[rgba(46,111,94,0.25)]">
                    ...
                    <h3 className="mt-6 max-w-lg font-display text-2xl font-semibold tracking-tight text-[var(--sl-ink)] sm:text-3xl">
                      AURA: The two-sided restaurant platform.
                    </h3>
                    <p className="mt-3.5 max-w-lg text-sm leading-relaxed text-[var(--sl-ink-soft)] sm:text-base">
                      Pairs an intuitive guest ordering app with a high-cadence kitchen console, designed as one unified system. Click to open the full interactive case study.
                    </p>

                    <div className="mt-8 flex flex-wrap gap-2">
                      {['Customer PWA', 'Kitchen Display', 'Live Synced State', 'Zustand + React'].map((tag) => (
                        <span
                          key={tag}
                  ...
                  <div className="md:col-span-5 flex flex-col items-center justify-center border-t border-[var(--sl-line)] bg-[var(--sl-sand-deep)]/30 p-6 sm:p-8 md:border-t-0 md:border-l z-10">
                    {/* Abstract "live system" visual */}
                    <div className="relative h-40 w-full max-w-[220px] sm:h-48">
                      {/* Window frame */}
                      <div className="absolute inset-0 rounded-xl border border-[var(--sl-line)] bg-white shadow-sm">
                        <div className="flex items-center gap-1.5 border-b border-[var(--sl-line-light)] px-3 py-2">
                          <span className="h-2 w-2 rounded-full bg-[var(--sl-oxblood)]/70" />
                          <span className="h-2 w-2 rounded-full bg-[var(--sl-gold)]/70" />
                          <span className="h-2 w-2 rounded-full bg-[var(--sl-teal-deep)]/70" />
                        </div>
                        <div className="space-y-2 p-3">
                          <div className="h-2 w-3/4 rounded-full bg-[var(--sl-line)]" />
                          <div className="h-2 w-1/2 rounded-full bg-[var(--sl-line-light)]" />
                          <div className="flex gap-2 pt-1">
                            <div className="sl-float-a h-5 w-14 rounded-md bg-gradient-to-r from-[rgba(198,71,43,0.3)] to-[rgba(217,164,65,0.3)]" />
                            <div className="sl-float-b h-5 w-10 rounded-md bg-[var(--sl-line-light)]" />
                          </div>
                          <div className="flex gap-1.5 pt-1">
                            <div className="sl-float-a h-6 flex-1 rounded-md bg-[rgba(46,111,94,0.15)]" />
                            <div className="sl-float-b h-6 flex-1 rounded-md bg-[rgba(217,164,65,0.15)]" />
                          </div>
                        </div>
                      </div>
                      {/* Accent glint */}
                      <div className="sl-float-b absolute -right-1 -top-1 h-5 w-5 rounded-full bg-gradient-to-br from-[var(--sl-oxblood)] to-[var(--sl-gold)] shadow-md shadow-orange-950/20" />
                    </div>
                    <p className="mt-3.5 text-center text-[11px] text-[var(--sl-charcoal)]">
                      One system, both sides full live demo inside the case study.
                    </p>
                  </div>
                </div>
              </Link>
