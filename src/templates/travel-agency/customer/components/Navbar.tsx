      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-6">
          {navLinks.map((link) => (
            <NavLink
              ...
              to={link.to}
              className={({ isActive }) =>
                `text-sm font-medium transition-colors ${isActive ? 'text-[#F4B942]' : 'text-[#A8B0BA] hover:text-white'}`
              }
            >
              {link.label}
            </NavLink>
        ...
        <div className="flex items-center gap-3">
          {user ? (
            <div className="relative">
              ...
              >
                ...
                <ChevronDown size={14} className={`text-[#A8B0BA] transition-transform ${userMenuOpen ? 'rotate-180' : ''}`} />
              </button>
              ...
              <AnimatePresence>
                {userMenuOpen && (
                  <motion.div
                    ...
                    className="absolute right-0 top-full mt-2 w-52 bg-[#171E27] border border-white/10 rounded-2xl shadow-2xl overflow-hidden z-50"
                  >
                    <div className="p-3 border-b border-white/10">
                      <p className="text-sm font-semibold text-white">{user.name}</p>
                      <p className="text-xs text-[#A8B0BA]">{user.email}</p>
                    </div>
                    <div className="p-1.5">
                      <Link to={`${TRAVEL_BASE}/dashboard`} onClick={() => setUserMenuOpen(false)}
                        className="flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm text-[#A8B0BA] hover:text-white hover:bg-white/5 transition-colors">
            ...
            <div className="flex items-center gap-2">
              <Link to={`${TRAVEL_BASE}/login`}
                className="text-sm text-[#A8B0BA] hover:text-white transition-colors px-3 py-2 hidden sm:block">
                Sign In
              </Link>
          ...
          <button
            ...
            className="md:hidden text-[#A8B0BA] hover:text-white p-2"
          >
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            className="md:hidden border-t border-white/[0.06] bg-[#0D1117]/95 backdrop-blur-xl"
          >
            <div className="px-4 py-4 flex flex-col gap-1">
              {navLinks.map((link) => (
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `px-4 py-3 rounded-xl text-sm font-medium transition-colors ${isActive ? 'bg-[#F4B942]/10 text-[#F4B942]' : 'text-[#A8B0BA] hover:text-white hover:bg-white/5'}`
                  }
                >
                  {link.label}
                </NavLink>
              ...
              {!user && (
                <div className="flex gap-2 mt-2 pt-2 border-t border-white/10">
                  <Link to={`${TRAVEL_BASE}/login`} onClick={() => setMobileOpen(false)}
                    className="flex-1 text-center text-sm py-2.5 rounded-xl border border-white/10 text-[#A8B0BA]">
                    Sign In
                  </Link>
