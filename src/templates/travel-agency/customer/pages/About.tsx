...
export default function About() {
  return (
    <div className="min-h-screen bg-[#FDFAF5]">
      {/* Hero */}
      <section className="relative py-24 bg-[#F5F0E8] border-b border-[#E8E0D5] overflow-hidden">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {values.map((v, i) => (
              <motion.div key={v.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
                className="bg-[#FDFAF5] border border-[#E8E0D5] rounded-2xl p-6 text-center">
                ...
                <h3 className="text-[#1C1917] font-semibold mb-2">{v.title}</h3>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {team.map((member, i) => (
            <motion.div key={member.name} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
              className="bg-white border border-[#E8E0D5] rounded-2xl p-5 text-center hover:border-[#F4B942]/20 transition-colors">
              <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${member.color} flex items-center justify-center text-[#1C1917] font-bold text-xl mx-auto mb-4`}>
                {member.avatar}
              </div>
              <h3 className="text-[#1C1917] font-semibold">{member.name}</h3>
              <p className="text-[#F4B942] text-xs mb-2">{member.role}</p>
              <p className="text-[#78716C] text-xs leading-relaxed">{member.bio}</p>
            </motion.div>
          ))}
        </div>
      </section>
