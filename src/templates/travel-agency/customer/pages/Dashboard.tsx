...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-[#0D1117]">
      <section className="bg-[#151B23] border-b border-white/[0.06] py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <p className="text-[#A8B0BA] text-sm mb-1">Welcome back,</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-10">
          {[
            ...
            <motion.div key={action.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 + i * 0.1 }}>
              <Link to={action.to} className="flex items-center gap-4 bg-[#171E27] border border-white/[0.06] hover:border-[#F4B942]/30 rounded-2xl p-5 transition-all group">
                ...
                <div>
                  ...
                  <p className="text-[#A8B0BA] text-xs">{action.desc}</p>
                </div>
              </Link>
            </motion.div>
          ))}
          ...
          <div className="space-y-4">
            {userBookings.map((booking, i) => (
              <motion.div key={booking.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 + i * 0.1 }}
                className="bg-[#171E27] border border-white/[0.06] rounded-2xl p-5 flex flex-wrap items-center gap-4 justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-white font-semibold text-sm">{booking.packageTitle}</p>
                    <p className="text-[#A8B0BA] text-xs">{booking.destination} · {booking.startDate} – {booking.endDate}</p>
                    <p className="text-[#A8B0BA] text-xs">Ref: {booking.bookingRef}</p>
                  </div>
                </div>
                <div className="flex items-center gap-4">
            ...
            {userBookings.length === 0 && (
              <div className="text-center py-12 bg-[#171E27] rounded-2xl border border-white/[0.06]">
                <p className="text-4xl mb-3">✈️</p>
                <p className="text-white font-semibold">No bookings yet</p>
                <p className="text-[#A8B0BA] text-sm mt-1">Start your journey today</p>
