    ...
    <div className="flex flex-col h-full bg-[#0D1117] border-r border-white/[0.06]">
      ...
      <div className="flex items-center justify-between p-5 border-b border-white/[0.06]">
        <Link to={ADMIN_BASE} className="flex items-center gap-2.5">
          ...
          <div>
            <span className="font-bold text-white text-sm">VoyageAI</span>
            <p className="text-[10px] text-[#A8B0BA] -mt-0.5">Admin Console</p>
          </div>
        </Link>
        <button onClick={onClose} className="md:hidden text-[#A8B0BA] hover:text-white p-1">
          <X size={18} />
        </button>
      </div>
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${
                isActive
                  ? 'bg-[#F4B942]/15 text-[#F4B942] border border-[#F4B942]/20'
                  : 'text-[#A8B0BA] hover:text-white hover:bg-white/5'
              }`
            }
          >
            <item.icon size={16} />
      ...
      <div className="p-3 border-t border-white/[0.06] space-y-1">
        <a href={CUSTOMER_BASE} target="_blank" rel="noopener noreferrer"
          className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-sm text-[#A8B0BA] hover:text-white hover:bg-white/5 transition-all">
        ...
        {open && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              onClick={onClose} className="fixed inset-0 z-40 bg-black/60 backdrop-blur-sm md:hidden" />
