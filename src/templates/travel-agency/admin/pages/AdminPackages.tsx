    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[#A8B0BA] text-sm">{filtered.length} packages</p>
        </div>
                  ...
                  <td className="px-4 py-3">
                    {editId === pkg.id ? (
                      <input value={editForm.title || ''} onChange={e => setEditForm(f => ({ ...f, title: e.target.value }))}
                        className="bg-[#0D1117] border border-[#F4B942]/40 rounded-lg px-2 py-1 text-white text-xs w-40 focus:outline-none" />
                    ) : (
                      <div>
                        <p className="text-white font-medium text-sm line-clamp-1 max-w-44">{pkg.title}</p>
                        <p className="text-[#A8B0BA] text-xs">{pkg.nights}N/{pkg.duration}D</p>
                      </div>
                    )}
                  </td>
                  <td className="px-4 py-3 text-[#A8B0BA] text-xs">{pkg.destination}</td>
                  <td className="px-4 py-3 text-[#A8B0BA] text-xs">{pkg.duration} days</td>
                  <td className="px-4 py-3">
                    {editId === pkg.id ? (
                      <input type="number" value={editForm.price || ''} onChange={e => setEditForm(f => ({ ...f, price: Number(e.target.value) }))}
                        className="bg-[#0D1117] border border-[#F4B942]/40 rounded-lg px-2 py-1 text-white text-xs w-24 focus:outline-none" />
                    ) : (
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-1.5">
                      <button onClick={() => toggleFeatured(pkg.id)}
                        className={`text-[10px] px-1.5 py-0.5 rounded flex items-center gap-0.5 transition-colors ${pkg.featured ? 'bg-[#F4B942]/20 text-[#F4B942]' : 'bg-white/5 text-[#A8B0BA]'}`}>
                        <Star size={9} />F
                      </button>
                      <button onClick={() => toggleTrending(pkg.id)}
                        className={`text-[10px] px-1.5 py-0.5 rounded flex items-center gap-0.5 transition-colors ${pkg.trending ? 'bg-red-500/20 text-red-300' : 'bg-white/5 text-[#A8B0BA]'}`}>
                        <TrendingUp size={9} />T
                      </button>
                    </div>
                  </td>
                    ...
                    <div className="flex items-center gap-1.5">
                      {editId === pkg.id ? (
                        <>
                          ...
                          <button onClick={() => setEditId(null)} className="w-7 h-7 rounded-lg bg-white/5 hover:bg-white/10 flex items-center justify-center text-[#A8B0BA] transition-colors"><X size={13} /></button>
                        </>
                      ) : (
                        <>
        ...
        {deleteId && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }}
              ...
              <h2 className="text-white font-bold text-lg mb-1">Delete Package?</h2>
              <p className="text-[#A8B0BA] text-sm mb-5">This action cannot be undone.</p>
              <div className="flex gap-3">
                <button onClick={() => setDeleteId(null)} className="flex-1 border border-white/20 text-white py-2.5 rounded-xl text-sm hover:border-white/40 transition-colors">Cancel</button>
                <button onClick={confirmDelete} className="flex-1 bg-red-500 hover:bg-red-600 text-white font-semibold py-2.5 rounded-xl text-sm transition-colors">Delete</button>
              </div>
            </motion.div>
          </motion.div>
        )}
